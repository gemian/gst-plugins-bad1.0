/*
 * GStreamer Hybris/Mir video sink
...
/**
 * SECTION:element-hybrissink
 *
 *  The hybrissink creates its own window and renders the decoded video frames there.
 ...
 * |[
 * gst-launch -v filesrc ! qtdemux ! h264parse ! queue ! amcviddec-omxtiducati1videodecoder ! hybrissink
 * ]| test the video rendering with hybrissink
 * </refsect2>
 */
...
GST_DEBUG_CATEGORY (gsthybris_debug);
#define GST_CAT_DEFAULT gsthybris_debug

#if 0
static const char gst_hybrissink_sink_caps_str[] =
GST_VIDEO_CAPS_MAKE_WITH_FEATURES (GST_CAPS_FEATURE_MEMORY_MIR_IMAGE,
    ...
    GST_VIDEO_FORMATS_ALL);
     ...
     static GstStaticPadTemplate gst_hybrissink_sink_caps_template =
         GST_STATIC_PAD_TEMPLATE ("sink",
    GST_PAD_SINK,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS (gst_hybrissink_sink_caps_str));
#else
static GstStaticPadTemplate gst_hybrissink_sink_caps_template =
GST_STATIC_PAD_TEMPLATE ("sink",
    GST_PAD_SINK,
...
#define gst_hybris_sink_parent_class parent_class
G_DEFINE_TYPE (GstHybrisSink, gst_hybris_sink, GST_TYPE_VIDEO_SINK);

static void gst_hybris_sink_get_property (GObject * object,
    guint prop_id, GValue * value, GParamSpec * pspec);
static void gst_hybris_sink_set_property (GObject * object,
    guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_hybris_sink_finalize (GObject * object);
static void gst_hybris_sink_set_context (GstElement * element,
    GstContext * context);
static GstCaps *gst_hybris_sink_get_caps (GstBaseSink * bsink, GstCaps * filter);
static gboolean gst_hybris_sink_set_caps (GstBaseSink * bsink, GstCaps * caps);
static gboolean gst_hybris_sink_start (GstBaseSink * bsink);
static gboolean gst_hybris_sink_stop (GstBaseSink * bsink);
static gboolean gst_hybris_sink_preroll (GstBaseSink * bsink, GstBuffer * buffer);
static gboolean
gst_hybris_sink_propose_allocation (GstBaseSink * bsink, GstQuery * query);
static gboolean gst_hybris_sink_render (GstBaseSink * bsink, GstBuffer * buffer);
static gboolean gst_hybris_sink_query (GstBaseSink * bsink, GstQuery * query);

static void
gst_hybris_sink_class_init (GstHybrisSinkClass * klass)
{
  ...
  gobject_class->set_property = gst_hybris_sink_set_property;
  gobject_class->get_property = gst_hybris_sink_get_property;
  gobject_class->finalize = GST_DEBUG_FUNCPTR (gst_hybris_sink_finalize);
  gstelement_class->set_context = gst_hybris_sink_set_context;

  gst_element_class_add_pad_template (gstelement_class,
      gst_static_pad_template_get (&gst_hybrissink_sink_caps_template));

  gst_element_class_set_static_metadata (gstelement_class,
      "Hybris/Mir video sink", "Sink/Video",
      "Output to hybris/Mir surface", "Jim Hodapp <jim.hodapp@canonical.com>");

  gstbasesink_class->get_caps = GST_DEBUG_FUNCPTR (gst_hybris_sink_get_caps);
  gstbasesink_class->set_caps = GST_DEBUG_FUNCPTR (gst_hybris_sink_set_caps);
  gstbasesink_class->start = GST_DEBUG_FUNCPTR (gst_hybris_sink_start);
  gstbasesink_class->stop = GST_DEBUG_FUNCPTR (gst_hybris_sink_stop);
  gstbasesink_class->preroll = GST_DEBUG_FUNCPTR (gst_hybris_sink_preroll);
  gstbasesink_class->propose_allocation =
      GST_DEBUG_FUNCPTR (gst_hybris_sink_propose_allocation);
  gstbasesink_class->render = GST_DEBUG_FUNCPTR (gst_hybris_sink_render);
  gstbasesink_class->query = GST_DEBUG_FUNCPTR (gst_hybris_sink_query);
  ...
  frame_ready_signal =
      ...
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET (GstHybrisSinkClass, frame_ready_changed), NULL, NULL,
  ...
  surface_texture_client_set_signal =
      ...
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET (GstHybrisSinkClass, surface_texture_client_changed), NULL,
...
static void
gst_hybris_sink_init (GstHybrisSink * sink)
{
  GST_DEBUG_OBJECT (sink, "Initializing hybrissink");
  sink->pool = NULL;
...
static void
gst_hybris_sink_get_property (GObject * object,
    guint prop_id, GValue * value, GParamSpec * pspec)
{
  GstHybrisSink *sink = GST_HYBRIS_SINK (object);
...
static void
gst_hybris_sink_create_surface_texture (GObject * object)
{
  GstHybrisSink *sink = GST_HYBRIS_SINK (object);
  ...
  GST_DEBUG_OBJECT (sink, "Created new SurfaceTextureClientHybris instance: %p",
      sink->surface_texture_client);
  /* Because hybrissink is being loaded, we are definitely doing
   * hardware rendering.
   */
...
static void
gst_hybris_sink_set_property (GObject * object,
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  GstHybrisSink *sink = GST_HYBRIS_SINK (object);
  ...
  switch (prop_id) {
    case PROP_MIR_TEXTURE_ID:
      ...
      gst_hybris_sink_create_surface_texture (object);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
static void
gst_hybris_sink_finalize (GObject * object)
{
  GstHybrisSink *sink = GST_HYBRIS_SINK (object);
...
static void
gst_hybris_sink_set_context (GstElement * element, GstContext * context)
{
  GstHybrisSink *sink = GST_HYBRIS_SINK (element);
...
static gboolean
gst_hybris_sink_query (GstBaseSink * bsink, GstQuery * query)
{
  GstHybrisSink *sink = GST_HYBRIS_SINK (bsink);
...
static void
create_window (GstHybrisSink * sink, struct display *display, int width,
    int height)
{
  ...
  ua_ui_window_properties_set_titlen (window->properties, "HybrisSinkWindow", 13);
...
static GstCaps *
gst_hybris_sink_get_caps (GstBaseSink * bsink, GstCaps * filter)
{
  GstHybrisSink *sink;
  GstCaps *caps;

  sink = GST_HYBRIS_SINK (bsink);
...
static gboolean
gst_hybris_sink_set_caps (GstBaseSink * bsink, GstCaps * caps)
{
  GstHybrisSink *sink = GST_HYBRIS_SINK (bsink);
...
static gboolean
gst_hybris_sink_start (GstBaseSink * bsink)
{
  GstHybrisSink *sink = (GstHybrisSink *) bsink;
  ...
  if (!sink->surface_texture_client && sink->texture_id > 0) {
    ...
    gst_hybris_sink_create_surface_texture (G_OBJECT (bsink));
  }
#if 0
...
static gboolean
gst_hybris_sink_stop (GstBaseSink * bsink)
{
  GstHybrisSink *sink = (GstHybrisSink *) bsink;
...
gst_hybris_sink_preroll (GstBaseSink * bsink, GstBuffer * buffer)
{
  GST_DEBUG_OBJECT (bsink, "preroll buffer %p", buffer);
  return gst_hybris_sink_render (bsink, buffer);
}

static GstFlowReturn
gst_hybris_sink_render (GstBaseSink * bsink, GstBuffer * buffer)
{
  GstHybrisSink *sink = GST_HYBRIS_SINK (bsink);
...
{
  GST_DEBUG_CATEGORY_INIT (gsthybris_debug, "hybrissink", 0, " hybris video sink");

  return gst_element_register (plugin, "hybrissink", GST_RANK_MARGINAL,
      GST_TYPE_HYBRIS_SINK);
}

GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    hybrissink,
    "Hybris Video Sink", plugin_init, VERSION, "LGPL", GST_PACKAGE_NAME,
    GST_PACKAGE_ORIGIN)
